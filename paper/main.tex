...
\usepackage{url}
\usepackage{booktabs}
\title{Safety Filter Novelty Boundaries: ICLR Main-Conference Gate Archive}
\author{Anonymous Authors}
\begin{document}
\maketitle
\begin{abstract}
This document is not an ICLR main-conference submission. It is the v3 main-conference gate for a generated robotics draft about safe robot learning. The original research bet was: Identify when safety filters change the learned mechanism rather than merely clipping actions. After a stricter hostile review, the honest terminal decision is \textbf{KILL/ARCHIVE}. Submission-hardening version: v3.
\end{abstract}
...
\begin{itemize}
\item Safe robot arm with safe joint mechanism using nonlinear spring system for collision safety (2009)
\item Safe joint mechanism using inclined link with springs for collision safety and positioning accuracy of a robot arm (2010)
\item End-to-End Safe Reinforcement Learning through Barrier Functions for Safety-Critical Continuous Control Tasks (2019)
\item Transformer-based human-motion forecasting coupled with safe reinforcement learning for telepresence robot co-navigation (2026)
\item Safe robust multi-agent reinforcement learning with neural control barrier functions and safety attention mechanism (2025)
\end{itemize}
